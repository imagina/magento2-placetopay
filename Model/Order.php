...
class Order
{
    ...
    public function addNewOrderTransaction(
        ...
        $payment->setTransactionId($placetopayOrderId);
     ...
     * @return Sales\Order|false
     */
    public function loadOrderByPlacetpplOrderId($placetopayOrderId)
    {
        $orderId = $this->transactionResource->getOrderIdByPlacetpplOrderId($placetopayOrderId);
        if ($orderId) {
    ...
    public function setHoldedOrderStatus(Sales\Order $order, $status)
    {
        ...
        $order->addStatusHistoryComment(__('Placetopay status') . ': ' . $status);
        $order->save();
    }
     ...
     * @param float $amount
     */
    public function completePayment(Sales\Order $order, $amount, $placetopayOrderId)
    {
        $payment = $order->getPayment();
        $payment
            ->setParentTransactionId($placetopayOrderId)
            ->setTransactionId($placetopayOrderId . ':C')
